...
header('location:commentz.php?comments inserted successfully');

}


else{
header('location:commentz.php?comments were not inserted');
}
...
if(empty($_POST['names_user']) or empty($_POST['replied_comments'])){
  ...
  header('location:commentz.php?kindly insert names and reply');
  ...
  header('location:commentz.php?replies inserted successfully and you can now respond to texts22');
...
<div class="form-diving">
     ...
     <div class="form">
       <form action="commentz.php" method="post" id="form-boxing">
...
<div class="rep"><button class="reply-button4" onclick="setVisibility('reply-visible','none');">Close ViewReplies</button></div>
 ...
 <?php
  ...
  $sql_reply_user = "SELECT * FROM `login_user` WHERE email = '$email'";
  $result_reply_user = mysqli_query($con, $sql_reply_user);
  $num = mysqli_num_rows($result_reply_user) > 0;


  while ($row = mysqli_fetch_array($$result_reply_user)){
...
<div class="form-reply">
  <div id="showreplies-inputs"><!---------------------------------------showreplies-inputs-container-->
      ...
      <form action="commentz.php" method="post">
        <div class="input-field">
        <input type="text" name="names_user" class="placex" placeholder="Add Your Names @<?php echo $row['firstname'];?> <?php echo $row['lastname'];?>">
        </div>
<?php
  }
?>
